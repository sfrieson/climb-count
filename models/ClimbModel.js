class ClimbModel {
  ...
  addAttemptToCurrentSession(attemptData) {
    ...
    if (
      ...
      throw new Error("Color and result are required");
    }

    const attempt = {
      id: Date.now(),
      timestamp: new Date(),
      color: attemptData.color,
      routeId: attemptData.routeId || null,
  ...
  getColorStats(attempts = null) {
    ...
    attemptsToAnalyze.forEach((attempt) => {
      if (!colorStats[attempt.color]) {
        colorStats[attempt.color] = { success: 0, total: 0 };
      }
      colorStats[attempt.color].total++;
      if (attempt.success) {
        colorStats[attempt.color].success++;
      }
    });
